...
///////////////////////////////////////////////////////////////////////////////
	...
	*/
    ...
    /*
    {
        ...
        intent.putExtra( "SessionStatus", EmshBatteryStatus.SessionStatus ); 						// ??MCU????session???
        intent.putExtra( "HardwareStatus", EmshBatteryStatus.HardwareStatus );						// ?????????????????(0/1)
        intent.putExtra( "ProtocolVersion", EmshBatteryStatus.ProtocolVersion );					// ??????
        intent.putExtra( "DeviceModelNumber", EmshBatteryStatus.DeviceModelNumber );				// ????????
        intent.putExtra( "BatteryPowerMode", EmshBatteryStatus.BatteryPowerMode );					// ??????????
        intent.putExtra( "BatteryCapacityLevel", EmshBatteryStatus.BatteryCapacityLevel );			// ???????????????
        intent.putExtra( "BatteryCapacityPercent", EmshBatteryStatus.BatteryCapacityPercent );		// ????????????????(0%)
        intent.putExtra( "BatteryTemperatureStatus", EmshBatteryStatus.BatteryTemperatureStatus );	// ??????????
        intent.putExtra( "BatteryTerminalVoltage", EmshBatteryStatus.BatteryTerminalVoltage );		// ?????????(mV)
        intent.putExtra( "BatteryDischargeVoltage", EmshBatteryStatus.BatteryDischargeVoltage );	// ????????????(mV)
        intent.putExtra( "BatteryDischargeCurrent", EmshBatteryStatus.BatteryDischargeCurrent );	// ?????????????(mA)
        intent.putExtra( "LatestUpdateUnixTime", EmshBatteryStatus.tmLatestUpdate );				// ???????????
